<template>
  <div v-if="featureAuth">
    <BaseButton v-if="canCreate" buttonType="primary" @click="addRow" :disabled="isAddRowInProgress">추가</BaseButton>
    <BaseButton v-if="canDelete" buttonType="danger" @click="deleteItem">삭제</BaseButton>
    <BaseButton v-if="canUpdate" buttonType="primary" @click="saveData">저장</BaseButton>
    <BaseButton v-if="canExcel" buttonType="warning" @click="downloadExcel">엑셀 다운로드</BaseButton>
  </div>
</template>
...
const props = defineProps({
  ...
  featureAuth: { // 사용기능 → featureAuth (CUDE 문자열)
    type: String,
  }
});
