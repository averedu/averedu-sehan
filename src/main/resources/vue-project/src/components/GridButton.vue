<template>
  <div v-if="featureAuth">
    <BaseButton v-if="canCreate" buttonType="primary" @click="addRow" :disabled="isAddRowInProgress"
      >추가</BaseButton
    >
...
const saveData = () => {
  ...
  axios
    .delete(props.deleteUrl, { data: deleteRows })
    .then(() => {
...
const downloadExcel = () => {
  ...
  axios
    .get(props.downloadUrl, { responseType: 'blob' })
    .then((response) => {
